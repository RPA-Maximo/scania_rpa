...
# ── OSLC 字段选择（集中维护，两个查询函数共用）─────────────────────────────
# 规则（来自字段映射表）：
#   供应商国家（vencountry/vennation）不抓
#   联系人/联系电话/邮件/接收人/斯堪尼亚客户代码 不抓
...
def _fetch_poline_next_pages(
    headers: dict,
    poline_list: list,
    response_data: dict,
) -> list:
    """
    防御性处理 poline 分页：若 Maximo 对子表分页，跟踪 nextPage 链接合并所有行。

    Args:
        headers: HTTP 请求头（含认证）
        poline_list: 已抓到的 poline 行（会被修改并返回）
        response_data: 当前响应体（含顶层 nextPage 或内嵌 poline nextPage）

    Returns:
        list: 合并了所有分页的 poline 行列表
    """
    # Maximo OSLC 对单条资源（单个 PO）通常一次性返回全部 poline；
    # 此函数作为防御层，跟踪 nextPage 链接（若存在）。
    next_url = (
        response_data.get('nextPage')
        or response_data.get('oslc:nextPage')
        or (response_data.get('nextPage', {}) or {}).get('@id')
    )
    page = 1
    while next_url and page < 50:   # 最多 50 页防死循环
        page += 1
        try:
            resp = requests.get(
                next_url,
            ...
            if not items:
                break
            po = normalize_po_data(items[0])
            poline_list.extend(po.get('poline', []))
            next_url = (
                data.get('nextPage')
                or data.get('oslc:nextPage')
                or (data.get('nextPage', {}) or {}).get('@id')
            )
        except Exception:
            break
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if items:
                ...
                # 防御性处理 poline 分页（绝大多数情况 poline 一次性返回）
                poline = po.get('poline', [])
                if poline:
                    _fetch_poline_next_pages(headers, poline, data)
                    po['poline'] = poline
                ...
                line_count = len(po.get('poline', []))
                print(f"✓ ({elapsed:.1f}s, {line_count} 行)")
                return po
            else:
                print("未找到")
                return None
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if items:
                        ...
                        total_lines = sum(len(it.get('poline', [])) for it in items)
                        print(f"✓ {len(items)} 条 / {total_lines} 行明细")
